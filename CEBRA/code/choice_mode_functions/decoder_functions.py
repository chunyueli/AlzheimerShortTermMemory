...
def knn_decoder_function(embedding_train, label_train, embedding_test, label_test, arglist, K_num=1):
    ...
    Parameters:
        ...
        - label_train: dict with keys 'left_choice' and 'session_frame'. Values are ndarray, shape (n_samples,).
        ...
        - label_test: dict with keys 'left_choice' and 'session_frame'. Values are ndarray, shape (n_samples,).
    ...
    KNN_decoder.fit(embedding_train, label_train['left_choice'])
    ...
    train_label_reshape = label_train['left_choice'].reshape(-1, Trial_len)
    ...
    # use the trained KNN decoder to predict the choice label on testing dataset
    ...
    test_label_reshape = label_test['left_choice'].reshape(-1, Trial_len)
...
def knn_decoder_function_ablation(embedding_original, label, embedding_ablation, test_label, arglist, K_num=1):
    ...
    Parameters:
        ...
        - test_label: dict with keys 'left_choice', 'session_frame'. Values are ndarray, shape (n_samples,).
    ...
    KNN_decoder = cebra.KNNDecoder(n_neighbors=K_num, metric="cosine")
    KNN_decoder.fit(embedding_original, label['left_choice'])
    predicted_label_train = KNN_decoder.predict(embedding_original)
    predicted_label_train_reshape = predicted_label_train.reshape(-1, Trial_len)
    train_label_reshape = label['left_choice'].reshape(-1, Trial_len)
    ...
    test_label_reshape = test_label['left_choice'].reshape(-1, Trial_len)
...
def knn_decoder_distractor(KNN_decoder,  decoding_ACC_test, embedding_distractor, label_distractor, decoding_ACC_AVG_set, decoding_ACC_curve_dict, arglist):
    ...
    Parameters:
        ...
        - label_distractor: dict with keys 'left_choice' and 'session_frame'. Values are ndarray, shape (n_samples,).
    ...
    for key in embedding_distractor.keys():
        ...
        predicted_label_test_reshape = predicted_label_test.reshape(-1, Trial_len)
        test_label_reshape = label_distractor[key]['left_choice'].reshape(-1, Trial_len)
