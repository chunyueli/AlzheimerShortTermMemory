...
def split_train_test_data(data, label, ratio, random_state=42):
    ...
    Parameters:
        ...
        - label: dict. keys are 'session_frame', 'left_choice'. Values are ndarray, shape (n_samples,).
    ...
    Returns:
        ...
        - train_labels: dict. keys are 'session_frame', 'left_choice'. Values are ndarray, shape (n_samples_train,).
        - test_datas: ndarray, shape (n_samples_test, cell_number).
            Calcium neural activity of the testing set.
        - test_labels: dict. keys are 'session_frame', 'left_choice'. Values are ndarray, shape (n_samples_test,).
    """
    ...
    for i in range(2):
        trial_type_index = np.where(label['left_choice'] == i)[0]
...
def k_fold_split_data(data, label,  k_folds=5):
    ...
    Parameters:
        ...
        - label: dict
            Keys: 'session_frame', 'left_choice'
