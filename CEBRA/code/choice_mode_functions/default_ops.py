...
def default_ops():
    ...
    return {
        ...
        "sampling_cell_num": 145,  # Number of cells sampled from each brain region
        "behavior_outcome": ['correct_response', 'incorrect_response'],  # Choice mode: ['correct_response', 'incorrect_response']; Stimulus mode: 'correct_incorrect_response'
        "control_trial_num": 40,  # Minimum number of no-distractor trials (choice mode only)
        "distractor_trial_num": 4,  # Minimum number of trials per distractor type (choice mode only)
