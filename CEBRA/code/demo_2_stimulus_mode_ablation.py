...
from stimulus_mode_functions.combine_all_trials_all_brain_regions import dataset_all_sessions, stimulus_sampled_population, ablate_all_regions_except_one
from stimulus_mode_functions.split_dataset import split_train_test_data
from stimulus_mode_functions.plot_functions import decoding_accuracy_control_vs_app_mice_ablation, plot_bar_panel_control_vs_app_mice_decoding_accuracy_ablation, plot_p_value_significance
from stimulus_mode_functions.decoder_functions import knn_decoder_function_ablation, knn_decoder_distractor_ablation
from stimulus_mode_functions.bootstrap import compute_p_value
...
if __name__ == '__main__':
    ...
    distractor_relative_decoding_accuracy_summary = {mice: {seed_value: {distractor_type: {} for distractor_type in ['early', 'middle', 'late']} for seed_value in seed_array} for mice in arglist.mice_type}
    ...
    for mice in arglist.mice_type:
        ...
        for i, seed_num in enumerate(seed_array):
            data_seed, label_seed, brain_region_index = stimulus_sampled_population(all_subjects_data, all_subjects_label, seed_num, arglist)
    ...
    for mice in arglist.mice_type:
        for region in brain_region:
            ...
            index += 1
    plt.savefig(save_path_root / 'stimulus_mode_relative_decoding_accuracy_ablation.svg')
    # plt.close()
    ...
    plt.savefig(save_path_root / 'stimulus_mode_relative_decoding_accuracy_bar_graph_ablation.svg')
    # plt.close()
    ...
    plt.tight_layout()
    plt.savefig(save_path_root / 'stimulus_mode_relative_decoding_accuracy_ablation_p_value.svg')
