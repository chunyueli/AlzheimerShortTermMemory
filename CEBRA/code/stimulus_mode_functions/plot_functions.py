...
def plot_embedding_cross_validation(embedding, label, save_name, start_time=2, end_time=6, idx_order=(0, 1, 2)):
    ...
    # get trial_index
    left_control_ind = (label['Trial_type'] == 0)
    right_control_ind = (label['Trial_type'] == 1)
...
def plot_embedding(ax, embedding, label, data_type, mice_type, gray = False, idx_order = (0, 1, 2)):
    ...
    Returns:
        - ax: matplotlib.axes.Axes
    """
    l_ind = label['left_stimulus'] == 1 # left stimuli
    r_ind = label['left_stimulus'] == 0 # right stimuli

    if not gray:
        r_cmap = plt.cm.Reds  # Right stimuli colormap
        l_cmap = plt.cm.Blues  # Left stimuli colormap
