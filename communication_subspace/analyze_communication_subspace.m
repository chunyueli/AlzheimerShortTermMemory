function analyze_communication_subspace(group)

close all
clearvars -except group
clc

% Analyze communication subspace.
...
for animal_num = 1:numel(behavior)
    ...
    for session_num = 1:numel(behavior{animal_num})
        clearvars -except group behavior adj_concat_trial_by_trial_activity cell_num_thresh animal_num session_num optDimLambdaReducedRankRegress source_activity target_activity x y e
        
        for shuffle_num = 1:20
            disp(['Shuffle: ',num2str(shuffle_num)])
            clearvars -except group behavior adj_concat_trial_by_trial_activity cell_num_thresh animal_num session_num shuffle_num optDimLambdaReducedRankRegress source_activity target_activity x y e
            ...
            for source = 1:8
                for target = 1:8
                    ...
                    if ~isempty(X) && ~isempty(Y)
                        ...
                        cvLoss = [mean(cvl);std(cvl)/(cvNumFolds^0.5)];
                        
                        % To compute the optimal dimensionality for the regression model, call ModelSelect:
                        optDimLambdaReducedRankRegress{animal_num}{session_num}{source}{target}(shuffle_num) = ModelSelect(cvLoss,numDimsUsedForPrediction);
...
% Save.
save(['population_regression_',group,'.mat'],'-v7.3')
